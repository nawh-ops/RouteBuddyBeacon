...
struct GPXExporter {
    static func generateGPX(from locations: [CLLocation]) -> String {
        ...
        var lastAnnotatedLocation: CLLocation? = nil
        let annotationDistance: CLLocationDistance = 75.0
        
        for location in locations {
            let lat = location.coordinate.latitude
            let lon = location.coordinate.longitude
            let time = formatter.string(from: location.timestamp)
            let shouldAnnotateQuodWords: Bool
            
            if let last = lastAnnotatedLocation {
                let distance = location.distance(from: last)
                shouldAnnotateQuodWords = distance >= annotationDistance
            } else {
                shouldAnnotateQuodWords = true
            }
            ...
            if shouldAnnotateQuodWords {
                lastAnnotatedLocation = location
            }
